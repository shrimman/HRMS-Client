...
export default function ManagerExpenseDetails() {
    ...
    const loadExpenseDetails = useCallback(async () => {
        if (!expenseId) {
            navigate('/manager/travel-expenses')
            return
        }
    ...
    const handleApprove = async () => {
        ...
        setIsApproving(true)
        try {
            const updatedExpense = await approveExpenseByManager(Number(expenseId), { 
    ...
    const handleReject = async () => {
        ...
        setIsRejecting(true)
        try {
            const updatedExpense = await rejectExpenseByManager(Number(expenseId), { 
        ...
        <div className={cn('flex-1 overflow-auto p-8 bg-background')}>
            <div className={cn('max-w-6xl mx-auto space-y-6')}>
                <div className={cn('flex items-center justify-between')}>
                    <Button
                        variant="ghost"
                        onClick={() => navigate('/manager/travel-expenses')}
                        className={cn('gap-2')}
                    >
                ...
                <ExpenseDetailsView
                  ...
                  travelLink={`/travel/${expense.travelPlanId}`}
                />
            </div>
        </div>
    )
